...
options = {
  ...
  rangefinder_sampling_ratio = 5.,  -- 距離センサーのサンプリング割合
  odometry_sampling_ratio = 4.,  -- オドメトリのサンプリング割合
  fixed_frame_pose_sampling_ratio = 1.,  -- 固定フレームのポーズのサンプリング割合
  imu_sampling_ratio = 10.,  -- IMUのサンプリング割合
...
TRAJECTORY_BUILDER_2D.max_range = 20.0  -- 距離測定の最大範囲（メートル）
TRAJECTORY_BUILDER_2D.missing_data_ray_length = 20.5  -- 欠損データの代わりに使用する距離（メートル）
