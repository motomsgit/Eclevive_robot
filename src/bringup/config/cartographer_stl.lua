...
options = {
  ...
  published_frame = "zed_camera_link",  -- 発行するフレームの名前
  ...
  pose_publish_period_sec = 1e-3,  -- ポーズの発行周期（秒）
  --trajectory_publish_period_sec = 30e-3,  -- 軌跡の発行周期（秒）
  trajectory_publish_period_sec = 10e-3,  -- 軌跡の発行周期（秒）
...
TRAJECTORY_BUILDER_2D.max_range = 25.0  -- 距離測定の最大範囲（メートル）
TRAJECTORY_BUILDER_2D.missing_data_ray_length = 25.1  -- 欠損データの代わりに使用する距離（メートル）
...
POSE_GRAPH.optimization_problem.local_slam_pose_rotation_weight = 5e4  -- ローカルSLAMの回転に対する重み
--POSE_GRAPH.optimization_problem.odometry_translation_weight = 1e5  -- オドメトリの平行移動に対する重み
POSE_GRAPH.optimization_problem.odometry_translation_weight = 1.5e5  -- オドメトリの平行移動に対する重み
--POSE_GRAPH.optimization_problem.odometry_rotation_weight = 1e5  -- オドメトリの回転に対する重み
POSE_GRAPH.optimization_problem.odometry_rotation_weight = 1.5e5  -- オドメトリの回転に対する重み
...
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 240  -- サブマップあたりの距離データの数
--TRAJECTORY_BUILDER_2D.motion_filter.max_distance_meters = 0.1  -- 最大距離のフィルタ（メートル）
TRAJECTORY_BUILDER_2D.motion_filter.max_distance_meters = 0.01  -- 最大距離のフィルタ（メートル）
--TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(0.04)  -- 最大角度のフィルタ（ラジアン）
TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(0.08)  -- 最大角度のフィルタ（ラジアン）

--POSE_GRAPH.optimize_every_n_scans = 0 --by motoms submapの中で滑りが起きているため、local SLAMの問題です。それでは、global SLAMをoffにして、チューニングの邪魔をしないようにしましょう。
